...
module.exports = function() {
    ...
    if (conf.proxyConf) {
        var proxyRegExp = compileRoute(conf.proxyConf.route);
        var host = conf.proxyConf.origin.replace(/http(s)*:\/\//, "").replace(/\//g, "")
        var httpsFlag = /https/.test(conf.proxyConf.origin)
        app.use('/', proxy(host, {
            ...
            filter: function(req, res) {
                return new RegExp(proxyRegExp).test(req.url);
            }
        }));
    }
